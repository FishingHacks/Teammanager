...
app.post('/api/login', (req, res) => {
	...
	const jwt = sign(
		user,
		process.env.SECRET || 'W0MuEaua:m9JbY}9pX!s?orO4PcsBxr"o^V?S1Dl`re{`.VIpO',
...
app.post(
	...
	(req, res) => {
		const body = req.body;
		if (body.bio && body.email && body.role && body.status && body.username) {
...
app.put('/api/task/:id', (req, res) => {
	if (
		...
		const _id = tasks.reduce((a, b) => (a > b.id ? a : b.id), tasks[0].id) + 1;
...
app.post('/api/user/me/update', (req, res) => {
	...
	users = users.map((el) =>
		el.id === id ? { ...el, username: req.body.username || el.username } : el,
	);
...
const task1: TaskGroup = {
	...
	tasks: [
		...
		},
		{
			assignedUsers: ['KeksDev'],
			completed: true,
			description: 'The halloween playlist for the Eytron Halloween update',
			importance: 0,
