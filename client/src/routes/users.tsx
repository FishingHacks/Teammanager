...
function EditUser({
    ...
    return (
        <>
            ...
            <Button
                ...
                onClick={() => {
                    ...
                    if (
                        ...
                        return setError("avatar is not a url");
                    const _u: User = {
                        bio,
                        email: mail,
                        id: data.id,
                    ...
                    setSubmitting(true);
                    fetch(
                        `${window.location.origin}/api/user/${data.id}/update`,
                        {
                            method: "post",
            ...
            <Space h="sm" />
            <UserInfo avatar={avatar} contact={contact} email={mail} name={name} title={role} />
        </>
    );
}
...
export default function Dashboard() {
    ...
    return (
        <>
            <ScrollArea style={{ width: "calc(100vw - 90px)", height: "100%" }}>
                <Card>
                    <Title>Users</Title>
                    <Button
                        onClick={() =>
                    ...
                    {typeof users === "object" && user.role === "admin" && (
                        <UsersRolesTable
                            ...
                            changeRole={(id, newRole) => {
                                ...
                                if (!_u) return;
                                fetch(
                                    `${window.location.origin}/api/user/${id}/update`,
                                    {
                                        method: "post",
                                        body: JSON.stringify({
                                            ..._u,
